...
class MaxOut(torch.nn.Module):
    ...
    def __init__(self, num_units: int, input_dim: int, output_dim: int) -> None:
        ...
        self.linear.weight = self.linear.weight.repeat(self.num_units, 1, 1)
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        # Repeat inputs
        outputs: torch.Tensor = inputs.unsqueeze(1).repeat(1, self.num_units, 1)

        # Compute outputs
        outputs = self.linear(outputs)

        # Compute max accross units
        outputs = outputs.view(outputs.shape[0], self.num_units, -1)
    ...
    #     # Repeat inputs
    #     grad_inputs: torch.Tensor = grad_output.repeat(1, self.num_units)
